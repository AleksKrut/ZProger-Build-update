...
<div class="container">
    <div align="center">
        <h1>🔐 Установка Proger Build (bspwm) на Arch Linux с шифрованием</h1>
...
pacman -S dhcpcd iwd</code></pre>
    ...
    <details>
        <summary><strong>💽 Разметка диска (UEFI GPT с шифрованием)</strong></summary>
        Просмотр дисков:
...
set 1 boot on
mkpart primary
# start: 513MiB, end: 100%
quit</code></pre>
    </details>
    <details>
        <summary><strong>🔒 Шифрование раздела</strong></summary>
        <pre><code>cryptsetup luksFormat /dev/sda2
# введите YES и дважды пароль
...
lvs</code></pre>
    ...
    <details>
        <summary><strong>📂 Подготовка разделов и монтирование</strong></summary>
        <pre><code>mkfs.ext4 /dev/mapper/main-root
...
mount /dev/sda1 /mnt/boot</code></pre>
    ...
    <details>
        ...
        <pre><code>pacstrap -K /mnt base linux linux-firmware base-devel lvm2 dhcpcd net-tools iproute2 networkmanager vim micro efibootmgr iwd</code></pre>
    </details>
    <details>
...
systemctl enable iwd.service</code></pre>
        <strong>Настройка mkinitcpio:</strong>
        <pre><code>micro /etc/mkinitcpio.conf</code></pre>
        Найдите строку <code>HOOKS=(base udev autodetect modconf kms keyboard keymap consolefont block filesystems fsck)</code> и замените на:<br>
        <code>HOOKS=(base udev autodetect modconf kms keyboard keymap consolefont block filesystems encrypt lvm2 fsck)</code><br>
        <pre><code>mkinitcpio -p linux</code></pre>
    </details>
    <details>
        <summary><strong>💿 Установка загрузчика</strong></summary>
        <pre><code>bootctl install --path=/boot
cd /boot/loader
micro loader.conf</code></pre>
        Содержимое <code>loader.conf</code>:
        <pre><code>timeout 3
default arch</code></pre>
        <pre><code>cd /boot/loader/entries
micro arch.conf</code></pre>
        Содержимое <code>arch.conf</code> (замените UUID на реальный):
        <pre><code>title Arch Linux by ZProger
linux /vmlinuz-linux
initrd /initramfs-linux.img
options rw cryptdevice=UUID=uuid_от_/dev/sda2:main root=/dev/mapper/main-root</code></pre>
